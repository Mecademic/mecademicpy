...
(https://www.mecademic.com/res/support).
...
- Argument names for `MoveLinVelTrf` and `MoveLinVelWrf` have been updated to
  ...
  Calls that use positional arguments are unaffected.

## Version 2.4.0 (May 2025)

### Migration Guide

When installing mecademicpy via pip (`pip install mecademicpy`), only essential dependencies are now installed.
Some advanced features (e.g., `robot.UpdateRobot`, `robot.StartLogging`) depend on additional packages that are no
longer installed by default.
To install all dependencies, use `pip install mecademicpy[full]`.
For more details, check the README.md.

The following API changes have been made compared to previous version: Consult the detailed list below for
specific modifications and make sure to adjust your code accordingly if needed:

- `GetCollisionStatus` now returns a `SelfCollisionStatus` object instead of `CollisionStatus`,
  to better reflect the robot’s TCP API.
  To retrieve the work zone status, use the new method `GetWorkZoneStatus`, which returns a `WorkZoneStatus` object;
- `GetRtInputState` and `GetRtOutputState` now have a "include_timestamp" argument which is False by default.
  They now return a list of integers instead of a `TimestampedData`.
  If you want the previous behavior, use `include_timestamp=True`;
- The "GetRt..." methods for cyclical real-time data (like `GetRtJointPos`) now operate synchronously by default.
  The request is sent to the robot and the response is awaited, ensuring the returned position is
  the current one, not just the last cyclic snapshot;
  If you prefer the non-blocking behavior and are fine with the last received cyclic data,
  you can use the optional parameter `synchronous_update=False`;
- `SetTorqueLimitsCfg` default mode has changed to `MX_TORQUE_LIMITS_MODE_DELTA_WITH_EXPECTED`.
...
- Beta support for robot variables (`CreateVariable`, `robot.variables.my_var=my_val`, etc.)
...
- Deprecated callbacks: `on_estop`, `on_estop_reset`, `on_estop_resettable`, `on_pstop2`, `on_pstop2_reset`,
  `on_pstop2_resettable`
- Deprecated methods: `WaitPStop2Reset`, `WaitPStop2Resettable`, `WaitEStopReset`, `WaitEStopResettable` and
  `ResetPStop2`
